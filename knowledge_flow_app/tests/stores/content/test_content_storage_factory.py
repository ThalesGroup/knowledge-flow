...
def test_get_local_content_store(monkeypatch, patch_app_context):
    """Test: returns LocalStorageBackend when type is 'local'."""

    patch_app_context("local")

    # Patch the local config to use a temp path
    monkeypatch.setattr(
        "knowledge_flow_app.stores.content.content_storage_factory.ContentStoreLocalSettings",
        lambda: type("DummySettings", (), {"root_path": "/tmp/test_path"}),
    )
...
def test_get_minio_content_store(monkeypatch, patch_app_context):
    ...
    monkeypatch.setattr(
        "knowledge_flow_app.stores.content.minio_content_store.Minio", lambda *args, **kwargs: DummyMinio()
    )
