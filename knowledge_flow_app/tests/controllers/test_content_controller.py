...
class TestMetadataController:
    ...
    def test_get_markdown_preview(self, client: TestClient, markdown_file, minio_content_store):
        ...
        minio_content_store.save_content(
            document_uid=markdown_file.get("document_uid"), document_dir=markdown_file.get("document_dir")
        )
        response = client.get(
    ...
    def test_download_document_success(
        self, client: TestClient, markdown_file, minio_content_store, opensearch_metadata_store, document1
    ):
    ...
    def test_download_document_not_found(self, client: TestClient):
        ...
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json().get("detail") == "Original input file not found for document non_existent_uid"
