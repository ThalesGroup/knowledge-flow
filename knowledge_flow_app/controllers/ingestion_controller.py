            ...
            def event_generator() -> Generator[str, None, None]:
                for filename, input_temp_file in preloaded_files:
                    ...
                    try:
                        ...
                        vectorization_response = self.output_processor_service.process(
                            output_temp_dir, input_temp_file, metadata
                        )
                        logger.info(
                            f"Post-processing completed for {filename}: {metadata} with chunks {vectorization_response.chunks}"
                        )
                        yield (
                            ProcessingProgress(
                                step=current_step,
                        ...
                        yield (
                            ProcessingProgress(
                                step=current_step, status=Status.ERROR, error=error_message, filename=file.filename
                            ).model_dump_json()
                            + "\n"
                        )
