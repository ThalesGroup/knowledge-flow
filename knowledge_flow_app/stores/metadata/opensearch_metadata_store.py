    ...
    def update_metadata_field(self, document_uid: str, field: str, value: any):
        try:
            ...
            response_meta = self.client.update(
                index=self.metadata_index_name, id=document_uid, body={"doc": {field: value}}
            )
            ...
            query_body = {
                "script": {"source": script_source, "lang": "painless", "params": {"value": value}},
            ...
            logger.info(
                f"[VECTOR INDEX] Field 'metadata.{field}' updated for all chunks with UID='{document_uid}' => {value}"
            )
    ...
    def get_all_metadata(self, filters_dict: dict):
        ...
        try:
            ...
            response = self.client.search(
                ...
                body={"query": query},
                _source=["document_name", "document_uid", "date_added_to_kb", "retrievable", "front_metadata"],
                size=1000,
            )
